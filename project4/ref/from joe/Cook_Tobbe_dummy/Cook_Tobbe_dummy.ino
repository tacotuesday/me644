...
void loop() {
  if (runonce = 1) {
    ...
    attach_servos(1); delay(100); /* orient_encoders(); */ delay(100);
    cc_left = 0;
    ...
    while (Ping_measure > max_range) { Ping_measure = distance_measure(); drive(100, 100); dt = read_encoders(); myPID.Compute(); Serial.println(F("Finding bridge wall...")); }
    ...
    while (cc_left < (e1 + 24) * (64 / 8)) { drive(left_spd, right_spd); dt = read_encoders(); myPID.Compute(); Serial.println(F("Crossing the bridge...")); }
    delay(100); drive(0, 0); delay(100); /*orient_encoders();*/ delay(100);

    // Move forward to find the treasure
    while (Ping_measure > max_range) { Ping_measure = distance_measure(); drive(left_spd, right_spd); dt = read_encoders(); myPID.Compute(); Serial.println(F("Finding treasure...")); }
    delay(100); drive(0, 0); delay(100); /*orient_encoders();*/ delay(100);
    ...
    while (cc_left < 24 * (64 / 8)) { drive(left_spd, right_spd); dt = read_encoders(); myPID.Compute(); Serial.println(F("Search complete!")); }
    delay(100); drive(0, 0);

...
unsigned long cmd[4] = {0};        // Initialize array to send to grabber bot
unsigned long n1, e1, e2, n2;      // For potential debugging purposes: declare variables that hold array values 
...
unsigned long duration, inches, n1, e1, e2, n2;
...
void loop() {
  if (runonce == 1) {
    ...
    while (Ping_measure > max_range) { Ping_measure = distance_measure(); drive(left_spd, right_spd); Serial.println("Finding bridge wall..."); }
    ...
    while (cc_left < 144) { drive(left_spd, right_spd); Serial.println("Aligning with bridge..."); }
    ...
    while (cc_left < 19 || cc_right < 19) { drive(left_spd - 25, right_spd + 25); Serial.println("Turning right..."); } // Counter values = spitball. Need to finetune.
    delay(100); drive(0, 0); delay(100); orient_encoders(); delay(100);
    cc_left = 0; cc_right = 0;

    // Cross the bridge to grabber bot's y-coordinate: e1 + 2'.
    while (cc_left < (e1 + 24) * (64 / 8)) { drive(left_spd, right_spd); Serial.println("Crossing the bridge..."); }
    delay(100); drive(0, 0); delay(100); orient_encoders(); delay(100);

    // Move forward to find the treasure
    while (Ping_measure > max_range) { Ping_measure = distance_measure(); drive(left_spd, right_spd); Serial.println("Finding treasure..."); }
    delay(100); drive(0, 0); delay(100); orient_encoders(); delay(100);
    ...
    while (cc_left < 24 * (64 / 8)) { drive(left_spd, right_spd); Serial.println("Search complete!"); }
    delay(100); drive(0, 0);
